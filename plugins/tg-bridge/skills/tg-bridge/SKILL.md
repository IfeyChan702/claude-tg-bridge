...
- **彈窗提問攔截(PreToolUse)**:`AskUserQuestion` 嘅選項彈窗 TG 度撳唔到,橋接緊嘅 session 會卡死等人撳 — 所以另有一個 PreToolUse hook(**必須同步,唔可以 `async: true`**)會 deny 呢個 tool,叫 Claude 改用文字列選項提問再結束 turn,行返「Stop hook → TG → 回覆數字 → poll 叫醒」嘅文字回路。冇 bind 嘅 session 一樣係 no-op,彈窗照常。
...
(以 plugin 方式安裝,hooks 已自動有,唔使改任何 settings.json。如果有人只係將 skill 資料夾 copy 入 `.claude/skills/` 冇裝 plugin,先需要手動將 Stop / Notification / PreToolUse(matcher `AskUserQuestion`)三組 hook 加入該 project 嘅 `.claude/settings.json`,command 一律係 `python3 <BRIDGE> hook`;Stop / Notification 可以 `"async": true`,**PreToolUse 必須同步**(async 嘅話 deny 輸出會被無視,彈窗照出)。注意 settings 喺 session 中途先建立嘅話,要重開 session 或開一次 `/hooks` 先生效。)
...
| session 停咗喺選項彈窗,TG 冇收到問題 | 該 project 未掛 PreToolUse hook(舊版設定)→ 照上節加返;已卡住嗰個彈窗要喺電腦 UI 手動答一次先甩到 |
